...
package com.ahfat.james.animation_in_class
...
import android.support.design.R.id.end
...
(context: Context) : View(context) {
    ...
    fun wander() {
        ...
        when (style) {
            ...
            NINJA -> {
                val disguy = getObjectAnimator(200, "alpha", 1.0f, 0.0f)
                val reapguy = getObjectAnimator(500,"alpha", 0.0f, 1.0f)
                val moveguy = getObjectAnimator(500, "fraction", 0.0f, 1.0f)
                moveguy.interpolator = AccelerateInterpolator()
                val playlist = AnimatorSet()
                playlist.play(disguy).with(moveguy)
                playlist.play(reapguy).after(disguy).after(3000)
                playlist.start()
            }
            WOWEE -> {
                val cX = this.getWidth()
                val cY = this.getHeight()
                val fRadius = Math.hypot(cX.toDouble(),cY.toDouble()).toFloat()
                val anime = getCircularAnimator(3000, this, cX, cY, 0.0f, fRadius)
                val disguy = getObjectAnimator(500, "alpha", 1.0f, 0.0f)
                val reapguy = getObjectAnimator(500,"alpha", 0.0f, 1.0f)
                val spinguy = getObjectAnimator(2500, "angle", 0.0f, 270.0f)
                val moveguy = getObjectAnimator(500, "fraction", 0.0f, 1.0f)
                moveguy.interpolator = AccelerateInterpolator()
                val playlist = AnimatorSet()
                playlist.play(disguy).with(spinguy).with(moveguy)
                playlist.play(reapguy).with(anime)
                playlist.start()
            }
            else -> {
            }
    ...
    private fun getCircularAnimator(time: Int, view: View, width: Int, height: Int, bRad: Float, eRad: Float): Animator {
        val animator = ViewAnimationUtils.createCircularReveal(view, width, height, bRad ,eRad ).apply { duration = time.toLong()}
        return animator
    }
